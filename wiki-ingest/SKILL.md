...
10. **실행 로그 남기기**: `90. Wiki\_ingest-log.md`에 이번 실행 1행을 append(맨 아래).
    - 파일이 없으면 헤더와 함께 새로 만든다:
      ...
      | 날짜 | 프로젝트 | 생성 | 갱신 | 백링크 | MOC 갱신 | 비고 |
      |------|----------|------|------|--------|----------|------|
      ```
    - 한 행 형식: `| 2026-06-19 | <프로젝트 폴더명> | <생성 수> | <갱신 수> | <백링크 수> | <갱신한 MOC 섹션 수> | <확인필요/모순 등 특이사항, 없으면 -> |`
    - **append만**: 기존 행은 절대 수정/삭제하지 않는다.
    - 날짜는 `currentDate`(YYYY-MM-DD). lint 단독 실행 시에는 비고에 `lint`로 표기하고 생성/갱신/백링크는 `0`.
