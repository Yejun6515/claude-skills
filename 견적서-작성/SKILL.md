...
## 인쇄 레이아웃 (2026-08-05 개정 — 글자 크기)
제출용 PDF에서 **본문 9.1pt**가 나오도록 마스터 칸 폭이 617pt로 조정돼 있다(이전 7.2pt, 게다가 SV 조건문이 우측에서 잘려 인쇄됐음). **칸을 넓히면 인쇄 축소율이 떨어져 글자가 다시 작아진다.**
- 조건문을 고쳐 쓸 때 한 줄 한도: **D열 시작 563pt(≈118자) / E열 시작 548pt** — 넘으면 경고 없이 잘림.
- 긴 조건문은 행을 추가하지 말고 **병합+자동줄바꿈**(E41·D59·D67·D68 방식). 행을 늘리면 셀 주소가 밀려 필드맵이 깨진다.
- 품목 행 추가·삭제 시 **페이지1(1~64행) 합계 ≤ 890pt**여야 2페이지 유지. 행 높이를 일괄로 줄일 때 **병합행(41·59·67·68)은 제외** — 줄이면 둘째 줄부터 조용히 잘린다.
- **마스터를 Excel에서 편집했으면** `new`→`fill` 산출물이 Excel에서 열리는지 반드시 확인. 외부 링크가 하나라도 생기면 openpyxl 산출물이 안 열린다(2026-08-05 마스터에서 외부링크 13개·정의된이름 528개 제거).
- 상세 규칙·칸 폭 표 → `assets/machine_fields.md` **E 섹션**.
