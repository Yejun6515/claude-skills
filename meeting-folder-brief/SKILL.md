...
- **폴더 자료 = 용어 기준 사전.** Tiro 요약/전사를 읽을 때는 §2에서 뽑아 둔 폴더 소스의 용어집을 옆에 두고 대조한다 — Tiro 표기와 소스 문서 표기가 다르면 **소스 문서 쪽이 정(正)**이며, `# Claude Code` 분석·HTML에는 소스 문서 용어로 쓴다(Tiro가 잘못 전사하는 경우 대비). 소스에도 없는 생소한 표기는 지어내 고치지 말고 "미확인"으로 보고.
