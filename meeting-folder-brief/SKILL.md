...
- `mcp__tiro__get_note`(include=`["summary","documents"]`)로 내용을 가져와 노트의 `# Tiro` 섹션에 1회 기입한다. **어느 쪽인지에 따라 처리가 다르다(`tiro-meeting-note` §1~§1.5와 동일):**
...
- **# Tiro 용어 치환 (2026-07-06 예준님 결정, 방식 B).** 소스 자료(폴더 문서·위키·볼트)로 **확정된 고유명사 오인식만** `# Tiro` 본문에서 정식 표기로 치환한다. 치환하면 frontmatter `Tiro Edited: corrected`(치환 없으면 `raw`), **치환 내역은 `# Claude Code`의 용어 보정표**에 남긴다(원문 표기는 보정표 + frontmatter의 Tiro 웹 원본으로 보존). **불확실·소스에 없는 표기는 치환하지 않고** 보정표에 "추정/미확인"으로만 기록. 문장 구조·내용에는 손대지 않는다.
...
- `note-digest` §3~4.5를 그대로 따른다:
  ...
  - **섹션 규칙** : 소스 폴더 바로가기 = `# Yejun's memo`, 내 분석 전부 = `# Claude Code`, Tiro 요약 = `# Tiro`(§3의 용어 치환 외 불변). HTML 링크는 `# Claude Code`의 `## 정리 자료 (HTML)`에 `file:///`로.
...
- **# Yejun's memo 불가침**; `# Tiro`는 §3의 확정 고유명사 치환만 허용(그 외 불변, `Tiro Edited`로 상태 표시). 위키 노트는 읽기 전용(생성은 `wiki-ingest`).
