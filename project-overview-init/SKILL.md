...
- §1.3 Contracts(계약 문서 인덱스)와 §4.3 Billing & Payment는 **빈 스캐폴드로만 생성**한다(계약 전 프로젝트면 비워둠). §1.3의 계약서 위치 링크는 예준님이 직접 채우고, §4.3의 계약 기준선(PO 총액·지급조건) 확정과 입금 이력 갱신은 [[payment-log]] 소관.
