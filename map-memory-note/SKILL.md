...
- frontmatter에 `Tiro: <webUrl>` 키를 추가하고, 본문 맨 아래 `## Tiro` 섹션에 요약본을 그대로 넣는다 (재요약 금지 — Tiro가 정리한 것을 옮기기만. 요약 안의 `##` 헤더는 `###`로 한 단계 내려 노트 구조에 맞춘다)
...
<Tiro 요약본 그대로 (## → ### 강등). 매칭된 녹음 없으면 이 섹션 생략>
```
