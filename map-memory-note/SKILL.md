...
- frontmatter에 `Tiro: <webUrl>` 키를 추가하고, 본문 맨 아래 `## 클대리 정리` 섹션에 요약을 **"옵시디언 회의록" 양식으로 정리해** 넣는다 — 내가 정리한 내용이므로 티대리가 아닌 클대리 정리에, 원문 추적은 frontmatter `Tiro` 링크로(2026-07-29 티대리 생략 규칙). 양식 스펙은 `tiro-meeting-note` §1.5와 동일 — 원문 프로즈 통째 붙여넣기 금지:
